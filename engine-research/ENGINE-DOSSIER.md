...
- Game / build / version: **Unreal Gold**, base 226b upgraded to **OldUnreal patch 227k** (version 227, subversion 11, engine built 2026-08-15). Includes Return to Na Pali.
  - ⚠️ **`Subversion: 11` and the release tag `v227k_NN` are TWO DIFFERENT NUMBERINGS and must never be compared.** The engine's own startup banner prints `Version: 227 Subversion: 11` and `Compiled: Aug 15 2026` on consecutive lines of the same run `[verified-numerically 2026-09-07, home PC `Unreal.log`]`, so nothing was misread — `Subversion:` is OldUnreal's internal counter, not the GitHub release suffix. Comparing them manufactured a contradiction that was chased for two days.
  - **The deployed release is `v227k_15`** `[verified-numerically 2026-09-08]`, established not by date arithmetic but by symbol sets: `v227k_12`'s `Core.lib` exports `?PostLoad@UProperty@@UEAAXXZ`, `v227k_15`'s does not, and the deployed `Core.dll` does not either. See §11's 2026-09-08 entry.
...
- **The draw-twice coverage gap does NOT apply at the render-device layer** `[inferred-static 2026-09-03, /sr]`. XIII/TEW lose ~8–51% of draws to a second transform path the *API* offers per draw; UE1's M2 lives inside our own `URenderDevice` (`VRGoldDrv`), where every world primitive arrives through the one interface with the same `FSceneNode` and we own the shaders that consume it — there is no second path for the engine to slip a draw through. Two transferable cautions survive anyway: exclude the 2D/ortho pass from any "rewrite every projection" logic (UE1 separates `Draw2DLine`/`DrawTile` from world draws, so this is answered by construction), and **watch for skybox/mirror/portal `FSceneNode`s that carry their own view per frame** — a mirror rendered from the wrong eye reads as a transform bug `[hypothesis]`; worth confirming before the M3 headset test.
...
- **✅ SETTLED 2026-09-03 (`/pd`, static, from the SDK's own sources): our screenshot gamma of 2.0 was wrong, and would have corrupted the fog-brightness calibration.** `ReadPixels(..., bGammaCorrectOutput)` applied a hardcoded `pow(x, 1/2.0)`, recorded as a `[hypothesis]` needing a check against stock. Checked, and **`[disproved 2026-09-03]`**. What stock ICBINDx11Drv actually does `[inferred-static 2026-09-03]`:
  ...
  - Its default mode `GM_XOpenGL` — the default **explicitly**, not merely by enum order: `Src/UnConfig.cpp:177` registers it as `AddByteProp(CPP_PROP(GammaMode), GM_XOpenGL, ...)` — is `if (Gamma == 1) return In; else pow(abs(In), 1/Gamma)` (`Shaders/PostFX/ResScaling_Common.h`).
  - The slider's default is **0.5** → `Gamma` is exactly **1.0** → **stock applies no correction at all at default settings.** That 0.5 is the shipped default, not just this install's value: it is `Brightness=0.500000` in **`Default.ini`** as well as in the live `Unreal.ini` (both line 194).
...
- **⛔️ 2026-09-04 (`/lm`, dev PC): the deployed `VRGoldDrv.dll` is ABI-INCOMPATIBLE with this PC's engine build — it will not load.** Setting `GameRenderDevice=VRGoldDrv.VRGoldRenderDevice` and launching gives a Windows loader error: **entry point `?PostLoad@UProperty@@UEAAXXZ` (MSVC-mangled `UProperty::PostLoad`, x64) could not be located** for `VRGoldDrv.dll`. The driver imports that symbol from the engine Core, but the deployed Core does not export it, so the DLL fails to load and no render device is created. `[verified-live 2026-09-04, n=1]` The file itself is fine (198,144 B, well-formed PE32+ x64) — the problem is **version/ABI skew**, not a missing or corrupt file. Likely cause: VRGoldDrv was built against a different 227k sub-version's headers/libs than the Core deployed in `System64\` (dossier §1 records the deployed build as 227 sub ~11; `MACHINES.md` lists the SDK as v227k_12). ⚠️ **"Deployed here" is NOT "works here"** — the board's dev-PC rows assumed the driver loads; it does not. Fix (`[PD]`): rebuild `VRGoldDrv` against the EXACT Core deployed here (confirm the running `Core.dll`/`Engine.dll` sub-version and link against its matching SDK), or align the deployed engine to the SDK the driver was built with. The dev-PC `[FLAT]` rows (blue-sky channel-swap, `VRGOLD STEREO 1` M2, gamma A/B) are all blocked until the driver loads. ⚠️ Also verify the home PC's engine build matches whatever the M2 build was validated against, or the same error will greet the `[FLAT @home]` M2 proof. Evidence: `dev-archive/recon/2026-09-04-vrgolddrv-abi-mismatch-uproperty-postload/`; notes `modding-notes/2026-09-04-vrgolddrv-abi-mismatch-blocks-the-dev-pc-launch.md`.

## 12. Open risks toward the North Star
- **✅ RESOLVED 2026-09-08 (`/pd`, dev PC, NO LAUNCH): the ABI blocker above is fixed, and the fix was one symbol wide.** `VRGoldDrv` was rebuilt against the **v227k_15** SDK and the loader error cannot recur. The chain, every link measured on files already on disk:
  1. The deployed `Core.dll` exports `PostLoad` for `UClass`, `UField`, `UFunction` and `UObject` — **but not for `UProperty`** `[verified-numerically 2026-09-08]`. That reproduces the 2026-09-04 live loader error statically, with no launch.
  2. The old driver imported **exactly one** symbol the deployed engine lacks, out of 207 imports across 6 DLLs — `?PostLoad@UProperty@@UEAAXXZ` `[verified-numerically 2026-09-08]`.
  3. **`v227k_12`'s `Core.lib` contains that symbol; `v227k_15`'s does not**, and neither does the deployed `Core.dll` `[verified-numerically 2026-09-08]`. That is what settles the version question — an independent *use* of the ABI rather than a second reading of a date.
  4. Rebuilt against v227k_15, the import set changes by **exactly one symbol**: `?PostLoad@UProperty@@UEAAXXZ` → `?PostLoad@UField@@UEAAXXZ` `[verified-numerically 2026-09-08]`. The override moved up to the base class between the two releases, and the deployed engine exports the base. Nothing else in the driver's ABI surface moved.
  5. The rebuilt driver imports **0** symbols the deployed engine does not export, across both `Core.dll` (100) and `Engine.dll` (23) `[verified-numerically 2026-09-08]`.

  ⚠️ **What this does NOT establish: that the driver works.** It establishes that the loader will not reject it for a missing entry point — which is precisely the failure that happened, and nothing more. A signature change hiding behind an unchanged mangled name is invisible to this check and would still fail at runtime. The dev-PC `[FLAT]` rows are unblocked, not answered.

  **The check is now a script, so this never costs a launch again:** `VRGoldDrv/check-abi.sh` diffs every symbol the driver imports against what the deployed `System64` DLLs export, and names the missing ones per source DLL. Run it after any build.

  **Two build defects fixed alongside it:**
  - The SDK **moved its import libraries** — `v227k_12` puts them in `<Package>/Lib64/`, `v227k_15` in `<Package>/Lib/x64/` (and split the download per platform, and roughly halved it). `CMakeLists.txt` now probes for both, so one file builds against either SDK. This corroborates the earlier `[inferred-static]` reading that the asset rename plus halving meant a restructured SDK, not a patch release.
  - Two builds of identical source differed in **2 bytes** — the PE `TimeDateStamp` at `0x118` and its copy in the debug directory at `0x27da4` — so `CONVENTIONS.md`'s "rebuild and compare the hash" check silently could not work on this project. Fixed with the MSVC linker's `/Brepro`; two builds are now **byte-identical, 0 differing bytes** `[verified-numerically 2026-09-08]`. Same defect, and the same day's fix, as `doom-2016-vr`'s `-Wl,--no-insert-timestamp`.

  **Deployed and stamped:** `304f2d2ce96d`, 198,144 B. The broken v227k_12 build is preserved as `System64\VRGoldDrv.dll.bak-2026-09-08-v227k_12-abi-broken`. The v227k_15 SDK is extracted at `C:\Users\Tefa\ue1-sdk-227k_15` on the dev PC (the v227k_12 copy at `ue1-sdk-227k` was left in place); **the home PC does not have it yet, and needs it before its `[FLAT @home]` M2 proof.**

  Host suites still pass against the rebuilt driver: `StereoMathTest` 70,550 checks and `GammaTest` 2,070 checks, 0 failures `[verified-numerically 2026-09-08]`.
