...
**Last `/gr` pass: 2026-09-04 (estate sweep) — CHECK-IN** (board OPEN block + INDEX)**.** Inbox empty; the `/sr` 2026-09-03 drop is still in `engine-research/inbox/`. **Nothing new, and not searched** — all three rows are `[FLAT]` and the modding side said on 2026-09-03 that none is a public-research question.
